...
LINK_PAGAMENTO = os.getenv("STRIPE_LINK")
...
def stripe_webhook():
    ...
    try:
        if event_type == 'invoice.paid':
            ...
            # 🔄 Fallback se o subscription_id estiver ausente
...
@app.route('/cancelar-assinatura', methods=['GET'])
def cancelar_assinatura():
    ...
    try:
        ...
        # ✅ Agenda o cancelamento para o fim do período atual
...
def assinar():
    ...
    try:
        checkout_session = stripe.checkout.Session.create(
            customer_email=email,
            payment_method_types=["card", "boleto"],  # se quiser incluir boleto aqui
            line_items=[{
                'price': STRIPE_PRICE_ID,  # Já configurado no seu .env
                'quantity': 1
            }],
            mode='subscription',
            success_url=f"{YOUR_DOMAIN}/?status=sucesso",
